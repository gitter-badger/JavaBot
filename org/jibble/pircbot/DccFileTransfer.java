		...
		if (!this._received) {
			...
			if (this._type.equals("SEND") && resume) {
				...
				if (this._progress == 0) {
					...
					this._bot.sendCTCPCommand(this._nick,
					        "DCC RESUME file.ext " + this._port + " "
					                + this._progress);
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					final int[] ip = DccFileTransfer.this._bot
					        .longToIp(DccFileTransfer.this._address);
					final String ipStr = ip[0] + "." + ip[1] + "." + ip[2]
					        + "." + ip[3];

					// Connect the socket and set a timeout.
					DccFileTransfer.this._socket = new Socket(ipStr,
					        DccFileTransfer.this._port);
					DccFileTransfer.this._socket.setSoTimeout(30 * 1000);
					DccFileTransfer.this._startTime = System
					        .currentTimeMillis();

					// No longer possible to resume this transfer once it's
					// underway.
					DccFileTransfer.this._manager
					        .removeAwaitingResume(DccFileTransfer.this);
				...
				DccFileTransfer.this._bot.onFileTransferFinished(
				        DccFileTransfer.this, exception);
			}
		}.start();
	}
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					InetAddress inetAddress = DccFileTransfer.this._bot
					        .getDccInetAddress();
					if (inetAddress == null) {
						inetAddress = DccFileTransfer.this._bot
						        .getInetAddress();
					}
					...
					// point ....
					String safeFilename = DccFileTransfer.this._file.getName()
					        .replace(' ', '_');
					safeFilename = safeFilename.replace('\t', '_');

					if (allowResume) {
						DccFileTransfer.this._manager
						        .addAwaitingResume(DccFileTransfer.this);
					}

					// Send the message to the user, telling them where to
					// connect to in order to get the file.
					DccFileTransfer.this._bot.sendCTCPCommand(
					        DccFileTransfer.this._nick, "DCC SEND "
					                + safeFilename + " " + ipNum + " "
					                + DccFileTransfer.this._port + " "
					...
					final BufferedInputStream input = new BufferedInputStream(
					        DccFileTransfer.this._socket.getInputStream());
					finput = new BufferedInputStream(new FileInputStream(
					        DccFileTransfer.this._file));
					...
					if (DccFileTransfer.this._progress > 0) {
						...
						while (bytesSkipped < DccFileTransfer.this._progress) {
							bytesSkipped += finput
							        .skip(DccFileTransfer.this._progress
							                - bytesSkipped);
						}
					}
