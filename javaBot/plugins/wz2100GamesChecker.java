...
public class wz2100GamesChecker extends javaBotPluginAbstract {
	...
	@Override
	public void run() {
		if (wz2100GamesChecker.message.equalsIgnoreCase(JavaBot.getPrefix()
		        + "wz2100")) {

			final String[] array = htmlParser
			        .readWebsite("http://wz2100.net/old-site/lobbyserver");
			int line = 0;
			...
			for (; line < array.length; line++) {    // Makes a loop to find the
				                                  ...
				                                  // warzone-content
				if (array[line]
				        .equalsIgnoreCase("<div class=\"warzone-content\">")) {
					break;
				}
			}
				...
				if (array[line].equalsIgnoreCase("<li>")) {
					for (; line < array.length; line++) {                      // Makes a loop to
						...
						if (array[line].contains("<strong>")) {
							String gameName = array[line].replaceAll(
							        ".*<strong>", "");
			...
			wz2100GamesChecker.bot.notice(wz2100GamesChecker.sender,
			        buffer.toString());
		}
	}
}
