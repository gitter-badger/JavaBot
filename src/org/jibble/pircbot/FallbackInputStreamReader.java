...
public class FallbackInputStreamReader extends Reader {
	...
	public FallbackInputStreamReader(PircBotLogger logger, InputStream in,
	        String primaryCharsetName, String fallbackCharsetName)
	...
	public int read(char[] cbuf, int off, int len) throws IOException {
		...
		if (FallbackInputStreamReader.debug) {
			this.logger.log(this.lastCharset + " READ return '"
	...
	private void fillDecodeBuffer() throws IOException {
		if ((this.decBuf != null) && this.decBuf.hasRemaining()) { return; }
		this.fillBuffer();
		if (this.buf == null) {
			this.decBuf = null;
			return;
	...
	private void fillBuffer() throws IOException {
		if (FallbackInputStreamReader.debug) {
			this.logger.log(this.buf == null ? "FISR bp null bl null fl n/a"
			        : ("bp " + this.buf.position() + " bl " + this.buf.limit()
		...
		if ((this.buf != null) && this.buf.hasRemaining()) {
			...
			this.logger.log("### BUG in FISR: pos<lim: " + this.buf.position()
			        + " < " + this.buf.limit());
			// discard remaining
		...
		try {
			while (true) {
				...
				if (r == -1) {
					...
					this.buf.flip();
					if (FallbackInputStreamReader.debug) {
						this.logger.log("FISR  -> 2 bp " + this.buf.position()
						        + " bl " + this.buf.limit() + " fl "
						        + this.bufFillLevel);
					}
				...
				if (this.buf.remaining() < r) {
					...
					this.buf.flip();
					final ByteBuffer bb = ByteBuffer.allocate(this.buf
					        .capacity() * 2);
					bb.put(this.buf);
					this.buf = bb;
				}
			...
			this.buf.limit(0);
			if (FallbackInputStreamReader.debug) {
				this.logger.log("FISR  -> " + e + " bp " + this.buf.position()
				        + " bl " + this.buf.limit() + " fl "
				        + this.bufFillLevel);
			}
			throw e;
	...
	private boolean checkLF(int start) {
		for (int i = start; i < this.buf.position(); ++i) {
			if (this.buf.get(i) == '\n') {
				...
				this.buf.limit(i + 1);
				if (FallbackInputStreamReader.debug) {
					this.logger.log("FISR  -> 1 / " + start + " bp "
					        + this.buf.position() + " bl " + this.buf.limit()
					        + " fl " + this.bufFillLevel);
				}
				return true;
			}
