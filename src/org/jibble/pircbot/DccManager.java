...
public class DccManager {
	...
	boolean processRequest(String nick, String login, String hostname,
		...
		if (type.equals("SEND")) {
			...
			final DccFileTransfer transfer = new DccFileTransfer(this._bot,
			        this, nick, login, hostname, type, filename, address, port,
			        size);
			...
			synchronized (this._awaitingResume) {
				for (int i = 0; i < this._awaitingResume.size(); i++) {
					transfer = (DccFileTransfer) this._awaitingResume
					        .elementAt(i);
			...
			if (transfer != null) {
				...
				this._bot.sendCTCPCommand(nick, "DCC ACCEPT file.ext " + port
				        + " " + progress);
			}
			...
			final DccChat chat = new DccChat(this._bot, nick, login, hostname,
			        address, port);
