...
public class DatabaseReader {
    ...
    public DatabaseReader(String name) {
        ...
        try {
            ...
            this.file = new ZipFile(System.getProperty("user.dir") + "/database/" + name + ".odb");
            ...
            while (this.en.hasMoreElements()) {
                ...
                if (this.ent.getName().startsWith("database/")) {
                    ...
                    this.out = new BufferedOutputStream(new FileOutputStream("/tmp/" + this.f.getName() + "."
                            + this.ent.getName().substring(9)));

                    // Add the newly created temp file to the tempfile vector
                    // for deleting
                    // later on
                    this.v.add("/tmp/" + this.f.getName() + "." + this.ent.getName().substring(9));
            ...
            // Obtain connection
            this.setCon(DriverManager.getConnection("jdbc:hsqldb:file:/tmp/" + this.f.getName(), "SA", ""));
